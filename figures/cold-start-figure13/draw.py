...
inds = np.arange(4)
cflow = list(pd.read_csv("CFlow-cold-start.csv")["cold_start_latency"])
faasflow = list(pd.read_csv("FaaSFlow-cold-start.csv")["cold_start_latency"])
dflow = list(pd.read_csv("DFlow-cold-start.csv")["cold_start_latency"][:4])
...
plt.yticks(np.array([0,5,10,60]), fontsize=20)
