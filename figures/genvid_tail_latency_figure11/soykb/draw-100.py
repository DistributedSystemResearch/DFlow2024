...
bar_width = 0.15
tick_label = ["8","16","24", "32", "40"]
# tick_label = ["10KB","100KB","1MB","10MB"]
inds = np.arange(len(tick_label))
cflow = list(pd.read_csv("raw_soykb_75MB.csv")["tail_latency"])
faasflow = list(pd.read_csv("optimized_soykb_75MB.csv")["tail_latency"])
dflow = list(pd.read_csv("optimized_soykb_75MB_DFlow.csv")["tail_latency"])
ideal = list(pd.read_csv("ideal.csv")["tail_latency"])[:5]
