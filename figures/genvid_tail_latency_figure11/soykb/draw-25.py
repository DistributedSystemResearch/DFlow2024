...
bar_width = 0.15
tick_label = ["4", "8","16","24"]
# tick_label = ["10KB","100KB","1MB","10MB"]
inds = np.arange(len(tick_label))
cflow = list(pd.read_csv("raw_soykb_25MB.csv")["tail_latency"])
faasflow = list(pd.read_csv("optimized_soykb_25MB.csv")["tail_latency"])
dflow = list(pd.read_csv("optimized_soykb_25MB_DFlow.csv")["tail_latency"])
ideal = list(pd.read_csv("ideal.csv")["tail_latency"])[:4]
...
# plt.text(0,-30,'(a) Soy-25MB/s', dict(size=20))]\
plt.title('(e)Soy-25MB/s', fontsize =25)
