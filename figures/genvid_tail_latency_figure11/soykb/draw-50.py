...
cflow = list(pd.read_csv("raw_soykb_50MB.csv")["tail_latency"])
faasflow = list(pd.read_csv("optimized_soykb_50MB.csv")["tail_latency"])
dflow = list(pd.read_csv("optimized_soykb_50MB_DFlow.csv")["tail_latency"])
...
plt.title('(f)Soy-50MB/s', fontsize =25)
# plt.xlabel('GPU Number', dict(size=22))
# my_ticks = np.array([0, 5, 15, 50, 40,55,60])
