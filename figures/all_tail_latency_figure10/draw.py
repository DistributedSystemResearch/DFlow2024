...
inds = np.arange(6)
cflow = list(pd.read_csv("CFlow-raw-6rpm-50MB.csv")["tail_latency"])
faasflow = list(pd.read_csv("FaaSFlow-optimized-6rpm-50MB.csv")["tail_latency"])
dflow = list(pd.read_csv("DFlow-optimized-6rpm-50MB.csv")["tail_latency"])
...
my_labels = ["CFlow", "FaaSFlow", "DFlow","Ideal"]
...
plt.savefig("all-tail-latency.pdf", bbox_inches='tight')
