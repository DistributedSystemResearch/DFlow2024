...
def run_workflow(workflow_name, request_id):
    ...
    rep = requests.post(url, json=data) #把这个给gateway，WorkerSP模式下，gateway在storage node 
...
def analyze_workflow(workflow_name):
    ...
    while time.time() - start <= TEST_PER_WORKFLOW and total <= 102:
        ...
        e2e_latency = run_workflow(workflow_name, id) #运行work_flow
        if total > 2:
...
def analyze(mode):
    ...
    workflow_pool = ['fileprocessing', 'wordcount']
    data_overhead = []
