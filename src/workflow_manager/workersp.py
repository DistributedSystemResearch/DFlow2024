...
class WorkflowState:
    def __init__(self, request_id: str, all_func: List[str]):
        ...
        self.remote_func:Dict[str,bool] = {}
        for f in all_func:
            self.executed[f] = False
            self.parent_executed[f] = 0
            self.record_funcs[f] = False 
            self.remote_func[f] = False
...
class WorkerSPManager:
    ...
    def trigger_function(self, state: WorkflowState, function_name: str, no_parent_execution = False) -> None:
        ...
        """
        ####
        ip = self.func_ip[function_name]
        if ip == self.host_addr: 
            ...
            self.trigger_function_remote(state, function_name, ip, no_parent_execution)
    ...
    def trigger_function_local_lambda(self, state: WorkflowState, function_name: str, func_ip:str, no_parent_execution = False)->None:
        ...
        log.info("4.95,trigger_function_lambda,the func_ip:%s",func_ip)
    ...
    def run_function_lambda(self, state: WorkflowState, function_name: str, func_ip:str) -> None:
        ...
        log.info("8 run_function_lambda,the function_name:%s",function_name)
        jobs = []
        next_func_list = info['next']
        log.info("9 run_function_lambda,the next_func_list:%s",next_func_list)
        #这个函数是next_func_list=['upload'] ，此时up_
        if len(next_func_list) == 0:
            return 
        for next_func in next_func_list:
            next_func_info = self.get_function_info(next_func) 
            log.info("next_func_info:%s",next_func_info)
            if 'next' in next_func_info:
                next_next_func_list = next_func_info['next']
                log.info('9 run_function_lambda,the next_next_func_list:%s and len(next_next_func_list):%s',next_next_func_list,len(next_next_func_list))
                if len(next_next_func_list) == 0:
                    continue
                for next_next_func in next_next_func_list:
                    state.lock.acquire()
                    next_next_func_ip = self.func_ip[next_next_func] #函数ip
                    log.info("10 run_function_lambda,the next_next_func:%s and state.record_funcs[%s]:%s",next_next_func,next_next_func,state.record_funcs[next_next_func])
                    log.info("10.7, run_function_lambda,the next_next_func_ip:%s",next_next_func_ip)
                    if state.record_funcs[next_next_func] == False:
    ...
    def trigger_function_remote_lambda(self, state: WorkflowState, function_name: str, remote_addr: str, no_parent_execution = False) -> None:
        ...
        state.lock.acquire()
        if state.remote_func[function_name] == True:
            state.lock.release()
            return 
        state.remote_func[function_name] = True
        state.lock.release()
    ...
    def trigger_function_lambda_next(self,state:WorkflowState,function_name:str)->None:
        ...
        for next_func in next_func_list:
            ...
            for next_next_func in next_next_func_list:
                ...
                #
                # son_next_func_ip = self.func_ip[son_next_func]
                next_next_func_ip = self.func_ip[next_next_func] #得到ip
                ...
                if state.record_funcs[next_next_func] == False:#未被启动
                    state.record_funcs[next_next_func] = True
                    state.lock.release()
                    if next_next_func_ip == self.host_addr:
                        job  = gevent.spawn(self.trigger_function_local_lambda,state,next_next_func,next_next_func_ip,True)
