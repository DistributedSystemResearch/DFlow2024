...
def parse(config_path):
    ...
    print('function_info.py,the config_file:',config_file)
    with open(config_file, 'r') as f:
        config = yaml.safe_load(f)
        workflow_name = config['workflow'] #? 12-13,workflow的名字
        max_containers = config['max_containers'] #?12-13,workflow的最多容器数量
        for c in config['functions']: #? 12-13,config['functions']对应函数
            ...
            function_info.append(info) #list
