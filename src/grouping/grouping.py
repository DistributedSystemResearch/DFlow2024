...
import yaml
import uuid
from handle_global import  SendGloablInput
import sys
sys.path.append("..")
...
def save_grouping_config(workflow: component.workflow, node_info, info_dict, info_raw_dict, critical_path_functions):
    ...
    repo.save_function_info(info_dict, workflow.workflow_name + '_function_info') #将info_dict存在workflow.workflow_name + '_function_info'这个db里
...
def save_grouping_config_lambda(workflow: component.workflow, node_info, info_dict, info_raw_dict, critical_path_functions,info_ip_dict):
    #注释:参数info_ip_dict包含整个workflow的对应的函数名所在的ip
    #key是workflow_name,value也是一个字典，该字段的key为函数名，value为函数所在的ip
    print("info_ip_dict",info_ip_dict)
    repo = repository.Repository(workflow.workflow_name)
    repo.save_function_info(info_dict, workflow.workflow_name + '_function_info') #将info_dict存在workflow.workflow_name + '_function_info'这个db里
    repo.save_function_info(info_raw_dict, workflow.workflow_name + '_function_info_raw')
    #repo.save_basic_input(workflow.global_input, workflow.workflow_name + '_workflow_metadata') TODO:lambda:我选择将global_input保存在每台worker机器上
    repo.save_start_functions(workflow.start_functions, workflow.workflow_name + '_workflow_metadata')
    repo.save_foreach_functions(workflow.foreach_functions, workflow.workflow_name + '_workflow_metadata')
    repo.save_merge_functions(workflow.merge_functions, workflow.workflow_name + '_workflow_metadata')
    repo.save_all_addrs(list(node_info.keys()), workflow.workflow_name + '_workflow_metadata')
    repo.save_critical_path_functions(critical_path_functions, workflow.workflow_name + '_workflow_metadata')
    repo.save_function_ip(info_ip_dict,workflow.workflow_name + '_function_ip')
    repo.save_global_input(workflow.global_input,workflow.workflow_name + '_global_input',workflow.workflow_name)
    if len(workflow.global_input) != 0:
        SendGloablInput(workflow.global_input) #将workflow_global_input用request.post广播到所有机器上
        #! workflow.global_input的形势是dict,类似{'hello':3}
...
def get_grouping_config(workflow: component.workflow, node_info_dict):
    ...
    for node_name in workflow.nodes:
        ...
        #todo:,12-13一些修改建议,如果node有后续节点，记录其后续节点所在的ip
        ###todo,12-13当node运行完时,给这些ip发消息，告诉它们你们的前驱节点运行完了
        ###todo,所以要记录后续节点所在的ip
        ###todo,12-13,是不是也可以记录节点A的前驱节点，是否需要开启节点A的后续节点,在trigger_function_lambda这里
        ###todo,给前驱节点发消息，获得其状态是否运行完毕
        ...
        # function_info_ip_dict[node.name] = ip 
        # #update node.name为函数节点的ip
        #function_info_ipdict 存有对应的ip
        function_info_raw = {'function_name': node.name, 'runtime': node.runtime, 'to': 'DB', 'ip': ip_list[hash(node.name) % len(ip_list)],
                             'parent_cnt': workflow.parent_cnt[node.name], 'conditions': node.conditions}
        function_input = dict()
        function_my_input = []
        function_my_output = dict()
        ...
        for arg in node.input_files:
            ...
            function_input_raw[arg] = {'size': node.input_files[arg]['size'],
                                       ...
                                       'parameter': node.input_files[arg]['parameter'],
                                       'type': node.input_files[arg]['type']}
            function_my_input.append(arg) #记录input的key
        ...
        for arg in node.output_files:
            ...
            function_my_output[arg] = node.output_files[arg]['size']
        ...
        log.info("the node_name:%s and function_info['my_input']:%s and function_info['my_output']:%s", node_name,function_info['my_input'],function_info['my_output'])
    ...
    #这个function_info_ip_dict存有整个workflow的函数节点所在的ip
...
def get_grouping_config_lambda(workflow: component.workflow, node_info_dict):
    ...
    function_info_ip_dict ={} #记录函数节点所在的ip
    ...
    for node_name in workflow.nodes:
        ...
        function_info_ip[node.name] = ip #函数节点存有对应的ip
        ...
        function_info['output'] = function_output
        function_info['next'] = node.next
        function_info['prev'] = node.prev #记录前驱节点
        print("\n\n********function_info[prev]:",function_info['prev'],' ******and function_name:\n\n',node_name)
        function_info['my_input'] = function_my_input
        function_info['my_output'] = function_my_output
        print("\n************'workflow_name:",workflow_name, " the node_name:",node_name, "and function_info['my_input']:", function_info['my_input'],"and function_info['my_output'}:",function_info['my_output'],'-----------------\n\n')
        #log.info("the node_name:%s and function_info['my_input']:%s and function_info['my_output']:%s", node_name,function_info['my_input'],function_info['my_output'])
        function_info_raw['input'] = function_input_raw
        function_info_raw['output'] = function_output_raw
        function_info_raw['next'] = node.next
        function_info_dict[node_name] = function_info #function_info_dict是一个dict,key是节点名字，即函数名，value是一个函数的各种信息
        function_info_raw_dict[node_name] = function_info_raw
    
     #这个function_info_ip_dict存有整个workflow的函数节点所在的ip
    function_info_ip_dict[workflow_name] = function_info_ip
    # if successor contains 'virtual', then the destination of storage should be propagated
    for name in workflow.nodes:
        for next_name in workflow.nodes[name].next:
            if next_name.startswith('virtual'):
    ...
    return node_info_dict, function_info_dict, function_info_raw_dict, critical_path_functions,function_info_ip_dict
...
if __name__ == '__main__':
    ...
    for workflow_name in workflow_pool:
        ...
        node_info, function_info, function_info_raw, critical_path_functions,function_info_ips = get_grouping_config_lambda(workflow, node_info_dict)
        ...
        save_grouping_config_lambda(workflow, node_info, function_info, function_info_raw, critical_path_functions,function_info_ips)