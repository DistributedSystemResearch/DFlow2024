...
class Store:
    def __init__(self, workflow_name, function_name, request_id, input, output, to, keys, runtime, db_server, redis_server):
        ...
        os.mkdir('work')
        self.create_request_doc(self.request_id)


    def create_request_doc(self, request_id):
        if request_id not in self.db:
            self.db[request_id] = {}
        return 
    ...
    def put_to_mem(self, k, content_type):
        ...
        #print('put_to_men:',len(self.put_dict[k]))
    ...
    def put(self, output_result, output_content_type):
        print('len(output_result):',len(output_result))
